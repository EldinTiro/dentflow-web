...
export function InviteUserDrawer({ onClose }: Props) {
      ...
      <Drawer title="User Invited" onClose={onClose}>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Account created successfully. Share the credentials below with the new team member.
          </p>
  ...
  return (
    <Drawer title="Invite Team Member" onClose={onClose}>
      <form onSubmit={handleSubmit((v) => invite.mutate(v))} className="space-y-4">
        ...
        {invite.isError && (
          <p className="text-sm text-red-600">Failed to invite user. They may already have an account.</p>
        )}
